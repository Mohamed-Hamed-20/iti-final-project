...
class ReviewService {
  async createReview(userId: string, reviewData: Partial<IReview>) {
    ...
    if (reviewData.referenceType === 'course') {
      ...
      const instructorCourses = await courseModel.find({ instructorId: reviewData.referenceId });
      ...
      const hasEnrolledCourse = await EnrollmentModel.findOne({
        userId: userId,
        courseId: { $in: instructorCourses.map(course => course._id.toString()) },
        status: { $in: ['active', 'completed'] },
        paymentStatus: 'completed'
      });
      
      if (!hasEnrolledCourse) {
        throw new Error('You must take a course from this instructor to review them');
      }
    }
  ...
  async updateReview(reviewId: string, userId: string, updateData: Partial<IReview>) {
  ...
  async getReviewStats(referenceId: string, referenceType: 'course' | 'instructor') {
