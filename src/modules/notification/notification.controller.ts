...
export const createNotification = async (userId: string, targetId: string, type: string = 'course'): Promise<INotification> => {
...
export const getUserNotifications = async (req: AuthenticatedRequest, res: Response) => {
    const notifications: INotification[] = await Notification.find({ userId: req.user!._id })
        .sort({ createdAt: -1 });
